...
class TestRunnerReporter():
    ...
    # key: test
    # value: None-pending, 0-passed, 1-failed
    def __init__(self, reporter, TestClass=None):
        self._tests = dict()
        self._test_res_list = []
    ...
    def run(self):
        ...
        for tst in to_run:
            try:
                self._TestClass.set_up()
                self.reporter.report_test_started(tst)
                tst()
            except:
                self._tests[tst] = TestResult.FAILED
                t = TestResult(tst, TestResult.FAILED, traceback.format_exc())
                ...
                self.reporter.report_test_finished(t)
                self._TestClass.tear_down()
        self.reporter.report_all_finished(len(self.ran_tests()), len(self.passed_tests()), len(self.failed_tests()))
        return (len(self.ran_tests()), len(self.passed_tests()), len(self.failed_tests()))
